    ...
    async def list_by_organization(self, *, organization_id: int, limit: int, offset: int) -> Sequence[ContractActivityTable]:
        try:
            query = (
                ...
                .order_by(desc(col(ContractActivityTable.created_at)), desc(col(ContractActivityTable.id)))
                .limit(limit)
                .offset(offset)
            )
            ...
            raise InternalServerError("Error al listar auditoria de contrato") from e
