...
class TemplateContentSynchronizer:
    ...
    def _build_reference_marker_key(self, raw_marker: str) -> str:
        ...
        return "_".join(tokens) or normalized_marker or "campo_referencia"
    ...
    def _sync_field_type_and_placeholder(self, inferred_type, field, updates):
        """Ensure `field.type` and `field.placeholder` reflect the inferred type.

        - Update `updates['type']` when the inferred type differs from the current type.
        - Rebuild the placeholder when it is autogenerated or the resolved type changed.
        """
    ...
    def _find_exact_field_key(self, fields: list[TemplateField], allowed_keys: tuple[str, ...]) -> str | None:
        ...
        return next((field.key for field in fields if field.key in allowed), None)
    ...
    def _select_best_field_key(
        ...
        *,
        ...
        return None if len(best_keys) != 1 else best_keys[0]
